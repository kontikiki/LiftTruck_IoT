...
#define SERVER_TIME 50  //ThingSpeak sending time(minute-debug-)
...
int readPacketFromFlash() {
  ...
  while (i < pkt_num) {
    ...
    sprintf(buf, "%d-%d-%d %d:%d:%d vehicle acc_max : %0.2f, avg_acc : %0.2f", 2000 + (writtenPacket[i].active_time.g_year), writtenPacket[i].active_time.g_month, writtenPacket[i].active_time.g_day, writtenPacket[i].active_time.g_hours, writtenPacket[i].active_time.g_minutes, writtenPacket[i].active_time.g_seconds, writtenPacket[i].svg_max,writtenPacket[i].avg_svg);
    Serial1.println(buf);
    i++;
  }
