...
#ifndef FLASH_AS_EEPROM_yn_h
#define FLASH_AS_EEPROM_yn_h
...
typedef struct timestamp {
  uint8_t g_day, g_month, g_year, g_hours, g_minutes, g_seconds;
} timestamp;
...
typedef struct {
  union{
  uint32_t data[EEPROM_EMULATION_SIZE/4];
  EEPROMpacket packet[EEPROM_EMULATION_SIZE/(sizeof(EEPROMpacket))];
  }res;
...
class EEPROMClass {
 ...
 //   EEPROMpacket EEPROMpkt;
    ...
    void convert(int num,EEPROMpacket *data);
