...
public:
  ...
  void write(const void *data,int label) { write(flash_address, data, flash_size, label); }
  void erase()  { erase(flash_address, flash_size);}
  void erase(int label)                 { erase(flash_address, flash_size,label);       }
  void read(void *data,int label)        { read(flash_address, data, flash_size,label);  }
  ...
  void write(const volatile void *flash_ptr, const void *data, uint32_t size, int label);
  void erase(const volatile void *flash_ptr, uint32_t size);
  void erase(const volatile void *flash_ptr, uint32_t size,int label);
  void read(const volatile void *flash_ptr, void *data, uint32_t size);
   void read(const volatile void *flash_ptr, void *data, uint32_t size,int label);
